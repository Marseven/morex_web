...
const form = useForm({
    ...
    current_amount: props.debt.current_amount,
    due_date: props.debt.due_date ? props.debt.due_date.split('T')[0] : '',
...
<template>
    <Head :title="form.type === 'debt' ? 'Modifier la dette' : 'Modifier la créance'" />
    <AppLayout>
        <div class="max-w-xl mx-auto">
            <div class="flex items-center gap-4 mb-6">
                ...
                <h1 class="text-lg font-semibold text-theme-text-primary">{{ form.type === 'debt' ? 'Modifier la dette' : 'Modifier la créance' }}</h1>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="bg-theme-card border border-theme-border rounded-lg p-4 space-y-4">
                    ...
                    <div>
                        ...
                        <input v-model="form.name" type="text" required class="w-full bg-theme-surface border border-theme-border rounded-md px-3 py-2 text-sm text-theme-text-primary focus:border-theme-text-primary focus:ring-0 outline-none" />
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-theme-text-secondary uppercase tracking-wider mb-2">Montant initial (FCFA)</label>
                        <input v-model.number="form.initial_amount" type="number" required min="1" class="w-full bg-theme-surface border border-theme-border rounded-md px-3 py-3 text-xl font-semibold text-theme-text-primary focus:border-theme-text-primary focus:ring-0 outline-none" />
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-theme-text-secondary uppercase tracking-wider mb-2">Montant restant (FCFA)</label>
                        <input v-model.number="form.current_amount" type="number" required min="0" class="w-full bg-theme-surface border border-theme-border rounded-md px-3 py-2 text-sm text-theme-text-primary focus:border-theme-text-primary focus:ring-0 outline-none" />
                    </div>
                    ...
                    <div>
                        ...
                        <input v-model="form.contact_name" type="text" class="w-full bg-theme-surface border border-theme-border rounded-md px-3 py-2 text-sm text-theme-text-primary focus:border-theme-text-primary focus:ring-0 outline-none" />
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-theme-text-secondary uppercase tracking-wider mb-2">Téléphone</label>
                        <input v-model="form.contact_phone" type="tel" class="w-full bg-theme-surface border border-theme-border rounded-md px-3 py-2 text-sm text-theme-text-primary focus:border-theme-text-primary focus:ring-0 outline-none" />
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-theme-text-secondary uppercase tracking-wider mb-2">Note</label>
                        <textarea v-model="form.description" rows="2" class="w-full bg-theme-surface border border-theme-border rounded-md px-3 py-2 text-sm text-theme-text-primary focus:border-theme-text-primary focus:ring-0 outline-none resize-none"></textarea>
                    </div>
                ...
                <div class="flex gap-3">
                    ...
                    <button type="submit" :disabled="form.processing" class="flex-1 py-2 bg-theme-btn-primary-bg text-theme-btn-primary-text text-sm font-medium rounded-md hover:opacity-90 transition-colors disabled:opacity-50">Enregistrer</button>
                </div>
            </form>
        </div>
    </AppLayout>
